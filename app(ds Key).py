...
5. 对白归属必须准确
6. 画外音和内心独白要正确区分
7. 输出纯JSON，不要任何额外文字，不要用markdown代码块包裹"""
...
def parse_json_response(raw):
    """从 LLM 返回的文本中提取 JSON，容错处理各种格式问题"""
    raw = raw.strip()
...
with col_info:
    if not api_key:
        st.caption(f"⚠️ 请在左侧填入 {provider} API Key")
...
if generate and script_text and api_key:
    with st.spinner("🎬 正在生成分镜表，请稍候（约 30-60 秒）..."):
        try:
            ...
            st.error(f"⚠️ AI 返回的格式有误，请重试。错误详情：{e}")
        except Exception as e:
            err = str(e)
            if "authentication" in err.lower() or "auth" in err.lower():
                st.error(f"⚠️ {provider} API Key 无效，请检查")
            elif "rate" in err.lower():
                st.error("⚠️ API 调用频率超限，请等待 60 秒后重试")
            elif "balance" in err.lower() or "quota" in err.lower():
                st.error(f"⚠️ {provider} 账户余额不足，请充值")
            else:
